...
def process_split(split):
  ...
  tfrecord_filename = TFRECORD_BASENAME.format(
      EVAL_DATA_DIR, split, 'image', img_shard_idx, NUM_SHARDS)
  img_writer = tf.io.TFRecordWriter(tfrecord_filename)

  txt_shard_idx = 0
  tfrecord_filename = TFRECORD_BASENAME.format(
      EVAL_DATA_DIR, split, 'text', txt_shard_idx, NUM_SHARDS)
  txt_writer = tf.io.TFRecordWriter(tfrecord_filename)
  
  dataset = tf.data.Dataset.list_files(INPUT_FILES.format(split))
